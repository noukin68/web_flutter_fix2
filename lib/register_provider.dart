  ...
  Future<void> verifyEmail(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['error'] != null) {
          showErrorMessage(
              context, 'Ошибка подтверждения email: ${responseData['error']}');
        } else {
          isEmailVerified = true;
          if (isEmailVerified) {
            await finishRegisterUser(context);
          } else {
            showErrorMessage(context, 'Неверный код подтверждения');
          }
        }
      } else {
        showErrorMessage(context, 'Ошибка подтверждения email');
      }
    } catch (e) {
      showErrorMessage(context, 'Ошибка подтверждения email: $e');
    }
  }
